...
export async function createNodeSqliteDatabase(
  ...
  if (options.migrate !== false) {
    ...
    if (!systemColumns.some((column) => column.name === "password_hash_iterations")) {
      connection.exec(
        "ALTER TABLE cms_system_settings ADD COLUMN password_hash_iterations INTEGER NOT NULL DEFAULT 100000 CHECK (password_hash_iterations IN (100000, 150000, 210000))",
      );
    }
    connection.exec(
